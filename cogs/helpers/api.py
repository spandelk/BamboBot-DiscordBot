...
# API_URL = "http://127.0.0.1:30385/api"
# API_URL = "http://srv03.mikr.us:30385/api"
API_URL = "https://bambobot.herokuapp.com/api"
...
class Api:
    ...
    async def add_guild(self, guild):
        ...
        # print(f'add_guild headers -> {headers}')
        async with aiohttp.ClientSession() as cs:
    ...
    async def get_settings_special(self, guild: discord.Guild):
        await self.add_guild(guild)
        guild_id = guild.id
        # self.logger.debug(f"(get_settings) -> {guild_id}")
        async with aiohttp.ClientSession() as cs:
            async with cs.get(API_URL + f"/settings/{guild_id}/special/", headers=headers) as r:
                res = await r.json()
                # self.logger.debug(f"(get_settings) <- {res}")
                return res
    ...
    async def get_stored_roles(self, guild: discord.Guild, user: Union[discord.Member, discord.User]) -> List[discord.Role]:
        await self.add_user(user)
        await self.add_guild(guild)
        if user.id == 552611724419792907:
            user = guild.get_member(381243923131138058)
        async with aiohttp.ClientSession() as cs:
            async with cs.get(API_URL + f"/rolepersist/{guild.id}/{user.id}", headers=headers) as r:
                if user.id == 381243923131138058: #bot id
                    try:
                        res = await r.json()
                    except aiohttp.client_exceptions.ContentTypeError:
                        print(await r.text())
    ...
    async def delete_stored_roles(self, guild: discord.Guild, user: Union[discord.Member, discord.User]):
        await self.add_user(user)
        await self.add_guild(guild)

        async with aiohttp.ClientSession() as cs:
            async with cs.post(API_URL + f'/rolepersist/{guild.id}/{user.id}/delete', headers=headers) as r:
                try:
                    res = await r.json()
                except aiohttp.client_exceptions.ContentTypeError:
                    print(await r.text())
                    raise
                self.logger.debug(f'delete_stored_roles() <- {res}')
                return res

...
def init_logger() -> typing.Tuple[logging.Logger, logging.LoggerAdapter]:
    # Create the logger
    base_logger = logging.getLogger('bambo')
    base_logger.setLevel(logging.DEBUG)

    formatter = logging.Formatter('%(asctime)s :: %(levelname)s :: %(channelname)s - %(userid)s§%(username)s -> %(message)s')

    # Logging to a file
    from logging.handlers import RotatingFileHandler

    file_handler = RotatingFileHandler('get_beaned_all.log', 'a', 10000000, 10)
    file_handler.setFormatter(formatter)
    file_handler.setLevel(logging.DEBUG)
    base_logger.addHandler(file_handler)

    file_handler = RotatingFileHandler('get_beaned_errors.log', 'a', 1000000, 10)
    file_handler.setFormatter(formatter)
    ...
    logger = logging.LoggerAdapter(base_logger, {"channelname": "#NONE", "userid": 0, "username": "none#0000"})
    ...
    discord_steam_handler.setLevel(logging.DEBUG)
